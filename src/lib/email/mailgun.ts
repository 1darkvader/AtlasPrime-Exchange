...
export const emailService = {
  ...
  async sendPasswordResetEmail(email: string, resetToken: string): Promise<boolean> {
    ...
    try {
      const resetUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://atlasprime.trade'}/reset-password?token=${resetToken}`;
  ...
  async sendVerificationEmail(email: string, verificationToken: string): Promise<boolean> {
    ...
    try {
      const verifyUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://atlasprime.trade'}/verify-email?token=${verificationToken}`;
