...
export default function DerivativesPage() {
  ...
  useEffect(() => {
    console.log('🔍 Derivatives Auth State:', {
      isAuthenticated,
      hasUser: !!user,
      authLoading,
    ...
    const fetchWalletBalances = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('💰 Derivatives wallet balances loaded:', data.summary);
        }
      } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.orders) {
          ...
          console.log('📋 Derivatives orders loaded:', data.orders.length);
        }
      }
    } catch (error) {
  ...
  const handleOrderSuccess = async () => {
    // Refresh wallet balances and orders after successful order - ENHANCED
    if (isAuthenticated) {
      try {
        const token = localStorage.getItem('atlasprime_token');
        ...
        if (walletResponse.ok) {
          ...
          setWalletBalances(data);
          console.log('✅ Derivatives balances refreshed after order:', data.summary);
        }

        // Refresh orders
        await fetchOrders();
        console.log('✅ Derivatives orders refreshed after placement');
      } catch (error) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-emerald-500/5 rounded-lg border border-emerald-500/20">
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        {(isAuthenticated && user) ? `Long ${currentContract.underlying}` : 'Log In to Trade'}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-red-500/5 rounded-lg border border-red-500/20">
                    ...
                    <div className="space-y-3">
                      ...
                      >
                        {(isAuthenticated && user) ? `Short ${currentContract.underlying}` : 'Log In to Trade'}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-12">
                  {authLoading ? (
                    ...
                    <>
                      ...
                      <p className="text-xs text-gray-500">Your open positions will appear here</p>
                    </>
                  )}
                </div>
              )}
                ...
                <div className="flex flex-col items-center justify-center py-12">
                  {authLoading ? (
                    ...
                    <>
                      ...
                      <p className="text-muted-foreground text-sm mb-4">
                        No {activeTab === "history" ? "order history" : "trades"} yet
                      </p>
